      ...
      <a-row v-if="securityKeyFlag" style="margin-top:15px;">
        <a-table size="small" :columns="columns" :dataSource="securityArr" :pagination="pagination" bordered>
          <template slot="paramIpt" slot-scope="text, record">
            <a-input id="token" :value="text" :data-id="record.id" @change="authParamChange" />
          </template>
        </a-table>
      </a-row>
      <a-row v-if="securityKeyFlag" style="margin-top:15px;">
        <a-card title="Login">
          <a-row style="margin-top:15px;">
            <a-col :span="4">phone</a-col>
            <a-col :span="18"><a-input id="phone" /></a-col>
          </a-row>
          <a-row style="margin-top:15px;">
            <a-col :span="4">code</a-col>
            <a-col :span="18"><a-input id="code" type="password" /></a-col>
          </a-row>
            <a-row style="margin-top:15px;">
            <a-col :span="4"></a-col>
            <a-col :span="18"><a-button type="primary" @click="phoneLogin">登陆</a-button></a-col>
          </a-row>
        </a-card>
      </a-row>
...
export default {
  ...
  methods: {
    ...
    phoneChange(e){
      ...
      this.oauth.phone=e.target.value;
    },
    codeChange(e){
      this.oauth.code=e.target.value;
    },
    clientChage(e){
    ...
    phoneLogin(){
      ...
      if(KUtils.strBlank(phone)){
          this.$message.info('username can\'t empty!!!');
          return false;
        }
        if(KUtils.strBlank(code)){
          this.$message.info('password can\'t empty!!!');
          return false;
        }
        ...
        var requestConfig = {
          url: "http://192.168.1.109:8081/login/phoneLogin",
          header: {
            "X-Device-Type": "ios"
          },
          method: "post",
        ...
        debugInstance
          ...
          .then(res => {
            ...
            var pkId = input.getAttribute("data-id");
